...
export default function ConfigurarCuentaPage() {
  ...
  useEffect(() => {
    // Solo puede entrar aquí alguien con sesión activa (viene del callback)
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60 px-8 py-10 space-y-7">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-slate-500" style={{ fontFamily: "var(--font-inter)" }}>
                Elegí una contraseña para acceder al portal.
              </p>
              {userEmail && (
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-100 px-4 py-3 space-y-1">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-[0.08em]" style={{ fontFamily: "var(--font-inter)" }}>
                Requisitos
              </p>
              <ul className="space-y-0.5">
                {[
                  ...
                  <li
                    key={text}
                    className={`flex items-center gap-2 text-xs transition-colors ${ok ? "text-green-700" : "text-slate-400"}`}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`w-full rounded-lg border bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition-all focus:bg-white focus:ring-2 ${
                    confirm && confirm !== password
                      ? "border-red-300 focus:border-red-400 focus:ring-red-100"
                      : "border-slate-200 focus:border-[#1a2e24] focus:ring-[#1a2e24]/10"
                  }`}
                  style={{ fontFamily: "var(--font-inter)" }}
                />
                {confirm && confirm !== password && (
                  <p className="mt-1.5 text-xs text-red-600" style={{ fontFamily: "var(--font-inter)" }}>
              ...
              <button
                ...
                className="mt-1 w-full rounded-lg bg-[#1a2e24] py-3 text-sm font-semibold text-white transition-colors hover:bg-[#243d30] disabled:opacity-50 disabled:cursor-not-allowed"
                style={{ fontFamily: "var(--font-inter)" }}
              >
